...
**Major components:**
- **FrameGrabber**  
  Owns the camera interface and timestamps frames at acquisition time.

- **FrameQueue**  
  Thread-safe queue used to decouple capture and processing rates.

- **VisionProcessor**  
  Performs image processing (e.g., grayscale conversion, filtering, feature detection).

- **Tracker (optional)**  
  Applies smoothing or state estimation (e.g., Kalman filtering) to noisy
